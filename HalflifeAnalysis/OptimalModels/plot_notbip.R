...
for (serotype in serotypes) {
  for (region in regions) {
    ...
    commontree_file = file.path("Results", serotype, paste0(serotype, "_", region, "_commontrees.txt"))
    tree1_file = file.path("MCMC", paste0(serotype, "_P1.tree"))
    tree2_file = file.path(region, paste0(serotype, ".treefile"))
    output_dir = file.path("Plots/notbip", serotype)
    
    print(paste("Обрабатываем:", serotype, region))
    print(paste("Файл общих клад (plain):", commontree_file))
...
print("Анализ commontrees.txt завершён!")
