...
write.table(get_taxa_name(t, 23),file="tree_order_23.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 19),file="tree_order_19.csv",col.names=F, row.names=F)

info_upd = add_colors2meta("order_files.txt", "C.csv")
info_upd = info_upd %>% arrange(GBAC)
write.csv(info_upd, "metadata_upd_C.csv",row.names=F)
...
t = ggtree(tree_rooted, size=0.1) %<+% info_upd + 
  ...
  geom_cladelab(node = 23, label = "EAST-AFRICA", align = T,  
                offset = 0.05, textcolor = 'red', barcolor = 'red') +
  geom_cladelab(node = 25, label = "EURO-SA", align = T, 
                offset = 0.05, textcolor = 'blue', barcolor = 'blue') +
  geom_cladelab(node = 19, label = "AFRICA", align = T, 
