...
setwd("C:/Users/Mate_/Downloads/Diploma/virus-recombination/R_gradient_colors/A_topotypes")

#source("add_gradient_colors.R")
source("../modified_gradients.R")

tree = read.tree("VP1_A.treefile")
...
write.table(get_taxa_name(t, 422),file="tree_order_422.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 448),file="tree_order_448.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 424),file="tree_order_424.csv",col.names=F, row.names=F)


info_upd = add_colors2meta("order_files.txt", "A.csv")
info_upd = info_upd %>% arrange(GBAC)
write.csv(info_upd, "metadata_upd_A.csv",row.names=F)
...
t = ggtree(tree_rooted, size=0.1) %<+% info_upd + 
  ...
  geom_cladelab(node = 422, label = "ASIA", align = T,  
                offset = 0.05, textcolor = 'red', barcolor = 'red') +
  geom_cladelab(node = 448, label = "EURO-SA", align = T, 
                offset = 0.05, textcolor = 'blue', barcolor = 'blue') +
  geom_cladelab(node = 424, label = "AFRICA", align = T, 
                offset = 0.05, textcolor = 'green', barcolor = 'green') +
