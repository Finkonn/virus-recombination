...
trees = list.files(path = "no_similar_treefiles/", full.names = TRUE, pattern = ".treefile$")
meta_path = "metadata_no_similar.csv"
...
for (file in trees) {
  ...
  pdf_file <- paste0("no_similar_plots/topotypes_in_tip_labels/", basename(file), "_combined.pdf")
  
  ggsave(pdf_file, g_nolegend, height = 14, width = 8)
  
  
}

ggsave(paste0("no_similar_plots/topotypes_in_tip_labels/", basename(file), "_legend.png"), 
       plot = legend, height = 10, width = 7, dpi = 600)
ggsave(paste0("no_similar_plots/topotypes_in_tip_labels/", basename(file), "_legend.svg"), 
       plot = legend, height = 10, width = 7, dpi = 600)
