...
t = ggtree(tree_rooted, size=0.1) %<+% info + geom_tiplab(size = 0.2, aes(color=serotype)) +
  geom_text(aes(label=node), hjust=-.3, size=0.2)
t 

write.table(get_taxa_name(t, 1578),file="tree_order_1578.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 2123),file="tree_order_2123.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 2244),file="tree_order_2244.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 1581),file="tree_order_1581.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 1891),file="tree_order_1891.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 1870),file="tree_order_1870.csv",col.names=F, row.names=F)
write.table(get_taxa_name(t, 2101),file="tree_order_2101.csv",col.names=F, row.names=F)
...
t = ggtree(tree_rooted, size=0.1) %<+% info_upd + 
  ...
  geom_tiplab(size = 0.5, aes(color=label)) +
  
  geom_cladelab(node = 1578, label = "O", align = T,  
                offset = 0.2, textcolor = 'red', barcolor = 'red') +
  geom_cladelab(node = 2123, label = "A", align = T, 
                offset = 0.2, textcolor = 'blue', barcolor = 'blue') +
  geom_cladelab(node = 2244, label = "C", align = T, 
                offset = 0.2, textcolor = 'green', barcolor = 'green') +
  geom_cladelab(node = 1581, label = "Asia1", align = T, 
                offset = 0.2, textcolor = 'pink', barcolor = 'pink') +
  geom_cladelab(node = 1891, label = "SAT2", align = T, 
                offset = 0.2, textcolor = 'cyan', barcolor = 'cyan') +
  geom_cladelab(node = 1870, label = "SAT3", align = T, 
                offset = 0.2, textcolor = 'orange', barcolor = 'orange') +
  geom_cladelab(node = 2101, label = "SAT1", align = T, 
                offset = 0.2, textcolor = 'purple', barcolor = 'purple') +
...
#             label = "SAT1", align = T, offset = 0.2, color = 'purple', barcolor = 'purple', offset.text=.01) +
  ...
  coord_cartesian(xlim = c(0, max(tree_rooted$edge.length) + 1.2)) +
...
for (file in trees){
  ...
  ggsave(svg_file, g, height = 10, width = 7)
}
