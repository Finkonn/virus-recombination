...
meta_path = "table1_with_lineage.csv"
...
plot_tree_with_gradient_and_heatmap = function(tree_file, meta, serotype_colors, topotype_colors, lineage_colors) {
  ...
  # Lineage + рамка вокруг всего блока
...
for (file in trees) {
  ...
  ggsave(paste0("Images/HeatbarAndGradient/", basename(file), "_legend.png"), 
         plot = legend, height = 10, width = 7, dpi = 600)
  ggsave(paste0("Images/HeatbarAndGradient/", basename(file), "_legend.svg"), 
         plot = legend, height = 10, width = 7, dpi = 600)
}
