...
target_lineage_colors <- lineage_colors[names(lineage_colors) %in% target_lineages]
...
plot_tree_with_gradient_and_heatmap = function(tree_file, meta, serotype_colors, topotype_colors, lineage_colors, target_lineages) {
  tree = read.tree(tree_file)
  tree_rooted = midpoint.root(tree)
  
  info_subset <- info
  
  info_subset$Topotype <- case_when(
    info_subset$Topotype == "EUROPE-SOUTH-AMERICA-EURO-SA-SOUTH" ~ "EURO-SA",
    info_subset$Topotype == "EAST-AFRICA-1-EA-1" ~ "EA-1",
    info_subset$Topotype == "EAST-AFRICA-2-EA-2" ~ "EA-2",
    info_subset$Topotype == "EAST-AFRICA-3-EA-3" ~ "EA-3",
    info_subset$Topotype == "EAST-AFRICA-4-EA-4" ~ "EA-4",
    info_subset$Topotype == "I-NORTHWEST-ZIMBABWE-NWZ" ~ "I-NWZ",
    info_subset$Topotype == "I-SOUTHEAST-ZIMBABWE-SEZ" ~ "I-SEZ",
    info_subset$Topotype == "III-NORTHWEST-ZIMBABWE-NWZ" ~ "III-NWZ",
    info_subset$Topotype == "II-WESTERN-ZIMBABWE-WZ" ~ "II-WZ",
    info_subset$Topotype == "III-WESTERN-ZIMBABWE-WZ" ~ "III-WZ",
    info_subset$Topotype == "INDONESIA-1-ISA-1-1" ~ "ISA-1",
    info_subset$Topotype == "IV-EAST-AFRICA1-EA-1" ~ "IV-EA-1",
    info_subset$Topotype == "MIDDLE-EAST-SOUTH-ASIA-ME-SA" ~ "ME-SA",
    info_subset$Topotype == "V-East Africa-EA" ~ "V-EA",
    info_subset$Topotype == "VII-EAST-AFRICA-2-EA-2" ~ "VII-EA-2",
    info_subset$Topotype == "WEST-AFRICA-WA" ~ "WA",
    TRUE ~ info_subset$Topotype
  )
  
  unique_topotypes_abbreviated <- unique(info_subset$Topotype)
  ...
  heat_data = data.frame(
    "serotype" = info_subset$serotype,
    "Topotype" = info_subset$Topotype,
    "Lineage" = info_subset$lineage
  )
  rownames(heat_data) <- info_subset$GBAC
  
  t = ggtree(tree_rooted, size = 0.1) %<+% info_subset +
    ...
    scale_color_manual(values=info_subset$color, guide='none') + 
  ...
  modified_lineage_colors <- lineage_colors
  non_target_lines <- setdiff(names(lineage_colors), target_lineages)
  modified_lineage_colors[non_target_lines] <- "grey80" 
  ...
  t_with_lineage <- gheatmap(t_with_topotype, heat_data["Lineage"],
                             ...
                             color = NULL,
                             colnames = FALSE) +
    scale_fill_manual(values = modified_lineage_colors, 
                      name = "Lineage",
                      breaks = target_lineages, 
                      limits = target_lineages) 
...
for (file in trees) {
  g = plot_tree_with_gradient_and_heatmap(file, meta_path, serotype_colors, topotype_colors, lineage_colors, target_lineages) +
  ...
  png_file <- paste0("all_sequences/panasia12iran/", basename(file), "_combined.png")
  svg_file <- paste0("all_sequences/panasia12iran/", basename(file), "_combined.svg")
  
  
  ggsave(png_file, g_nolegend, height = 10, width = 7, dpi = 600)
  ggsave(svg_file, g_nolegend, height = 10, width = 7, dpi = 600)
  
  ggsave(paste0("all_sequences/panasia12iran/", basename(file), "_legend.png"), 
         plot = legend, height = 10, width = 7, dpi = 600)
  ggsave(paste0("all_sequences/panasia12iran/", basename(file), "_legend.svg"), 
         plot = legend, height = 10, width = 7, dpi = 600)
}
